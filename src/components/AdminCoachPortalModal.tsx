...
import { NewsCategory, AthleteRecord, EmailNotificationLog } from '../types';
...
export const AdminCoachPortalModal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'noticias' | 'atletas' | 'tempos' | 'recados' | 'emails'>('atletas');
  ...
  if (!coachManagerModalOpen) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white font-serif flex items-center gap-2">
                <span>Painel da Coordenação & Treinadores</span>
                <span className="px-2 py-0.5 rounded-full bg-[#d4af37]/20 text-[#f3e5ab] text-[10px] font-mono font-bold">
                  Gestão Total ACEDEP
                </span>
              </h3>
              <p className="text-xs text-slate-400">
                Cadastro simplificado de atletas com foto, login individual, notícias e disparo automático de e-mails
              </p>
            </div>
          </div>

          <button
            onClick={() => setCoachManagerModalOpen(false)}
        ...
        {/* Tabs Bar */}
        <div className="px-6 pt-3 pb-2 bg-[#0a192f] border-b border-[#1e3a5f]/60">
          <div className="flex flex-wrap gap-1.5 p-1 bg-black/40 rounded-xl border border-[#1e3a5f]">
            ...
            >
              ...
              <span>Gerenciar Atletas ({athletes.length})</span>
            </button>
            ...
            >
              ...
              <span>E-mails aos Pais ({getAllParentEmails().length})</span>
            </button>
            ...
            >
              ...
              <span>Publicar Notícia ({newsPosts.length})</span>
            </button>
